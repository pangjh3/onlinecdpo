...
class OnlineCDPOTrainer(Trainer):
    r"""
    https://arxiv.org/pdf/2409.20370
    Initialize OnlineCDPOTrainer.
    ...
    def tokenize_row(feature, is_encoder_decoder: bool, tokenizer: PreTrainedTokenizerBase) -> Dict[str, Any]:
        ...
        if "answer" in feature:
            answers = tokenizer(feature["answer"], add_special_tokens=False)
            answers["input_ids"] =  answers["input_ids"] + [tokenizer.eos_token_id]
            answers["attention_mask"] =  answers["attention_mask"] + [0]
            batch.update({f"answer_{key}": value for key, value in answers.items()})
        return batch
    ...
    def training_step(
        ...
        prompt_ids = inputs["prompt_input_ids"].repeat(self.sample_k, 1)
        prompt_ids_plusone = inputs["prompt_input_ids"].repeat(self.sample_k+1, 1)
        prompt_mask = inputs["prompt_attention_mask"].repeat(self.sample_k, 1)
        prompt_mask_plusone = inputs["prompt_attention_mask"].repeat(self.sample_k+1, 1)
        solution_ids = inputs["answer_input_ids"]
        # print(solution_ids)
        # print("{{}}{{}}}{}}")
        ...
        _, a_len = completion_ids.shape
        bs, b_len = solution_ids.shape
        
        max_dim = max(a_len, b_len)
        # 在 dim=1 补零
        completion_ids = torch.nn.functional.pad(completion_ids, (0, max_dim - a_len), value=self.processing_class.pad_token_id)  # 右侧补零
        solution_ids = torch.nn.functional.pad(solution_ids, (0, max_dim - b_len), value=self.processing_class.pad_token_id)  # 右侧补零

        m, c_dim = completion_ids.shape
        n, s_dim = solution_ids.shape
        assert c_dim == s_dim

        # result = torch.zeros(m + n, c_dim).to(completion_ids)  # 最终结果的大小是 m + n
        # idx_a = torch.arange(m)
        # idx_b = torch.arange(0, m + n, self.sample_k + 1)
        # result[idx_a + torch.arange(m) // self.sample_k] = completion_ids  # 插入 a 的部分
        # result[idx_b] = solution_ids  # 插入 b 的部分

        # 构建插入索引
        chunks = completion_ids.view(n, self.sample_k, c_dim)  # 将 a 按每 k 个向量分成 n 个块
        result = torch.cat([torch.cat([chunk, solution_ids[i].unsqueeze(0)], dim=0) for i, chunk in enumerate(chunks)], dim=0).to(completion_ids)


        completion_ids = result
        # print(idx_a + torch.arange(m) // self.sample_k)
        # print(idx_b)
        ...
        prompt_completion_ids = torch.cat((prompt_ids_plusone, completion_ids), dim=1)
        prompt_completion_mask = torch.cat((prompt_mask_plusone, completion_mask), dim=1)
        ...
        gt_completion = self.processing_class.batch_decode(solution_ids, skip_special_tokens=True)
        ...
        if self.judge is not None:
            ...
            pos_neg_k_results = []
            for i in range(self.sample_k+1):
                pos_neg_k_results.append(self.judge.judge(
                    prompts, completions[i*num_examples:(i+1)*num_examples], gt_completion
                ))
            pos_neg_k_results = torch.Tensor(pos_neg_k_results)
            # pos_neg_completion = self.judge.judge(
            #     prompts, completions, gt_completion
            # )

            # convert ranks to a True/False mask:
            # when rank == 0, it means the first completion is the best
            # when rank == 1, it means the second completion is the best
            # mask = torch.tensor([rank == 1 for rank in pos_neg_completion], device=device)
        else:
        ...
        # print(pos_indices)
        # print(neg_indices)
        # x_indics = torch.arange(num_examples, device=device)
        # pos_indices = torch.arange(num_examples, device=device)
        # neg_indices = torch.arange(num_examples, device=device)
        # for i in range(num_examples):
        #     flag=0
        #     if pos_neg_k_results[0][i] == 1:
        #         flag = 1
        #     for k in range(1, self.sample_k):
        #         if pos_neg_k_results[k][i] == 0 and flag == 1:
        #             neg_indices[i] += (k*num_examples)
        #             break
        #         if pos_neg_k_results[k][i] == 1 and flag == 0:
        #             pos_indices[i] += (k*num_examples)
        #             break

        # if pos_indices.shape[0] == 0:
        #     return 0.0

        # min_sample = pos_indices.shape[0] if pos_indices.shape[0] < neg_indices.shape[0] else neg_indices.shape[0]
        # print(pos_indices)
        # print(neg_indices)
        # print(prompt_completion_ids[pos_indices])
        # print(prompt_completion_ids[neg_indices])
        chosen_indices = pos_indices[:]
        rejected_indices = neg_indices[:]
